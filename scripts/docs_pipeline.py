...
def _resolve_include_path(include_expr: str, source_file: Path, include_base_dir: Path) -> Path:
    """Resolve includes using the mdx-include base-path behavior first."""
    candidates = [
...
def run_zensical(
    *,
    ...
    try:
        ...
        raise DocsPipelineError(
            "The 'zensical' CLI was not found in PATH. "
            "Install docs dependencies and ensure the command is available."
        ) from exc
